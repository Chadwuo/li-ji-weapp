...
<template>
  <div class="min-h-screen bg-[#f8f5f0]">
    ...
    <div class="bg-white shadow-sm">
      <uv-calendars insert color="#F87171" confirm-color="#F87171" lunar @month-switch="onCalendarChange" />
    </div>
    ...
    <div class="mx-4 mt-4 rounded-2xl bg-white p-4 shadow-sm">
      <div class="mb-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="relative h-6 w-11 cursor-pointer rounded-full transition-colors duration-200"
               :class="weekendOnly ? 'bg-[#c9463d]' : 'bg-gray-300'" @click="weekendOnly = !weekendOnly"
          >
            <div class="absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform duration-200"
      ...
      <div class="grid grid-cols-4 justify-center gap-2">
        <div v-for="event in eventTypes" :key="event.key"
             class="flex cursor-pointer items-center gap-1.5 rounded-full px-2 py-1.5 transition-all duration-200" :class="[
    ...
    <div class="mx-4 mt-4 pb-8">
      ...
      <div v-else class="space-y-3">
        <div v-for="(day, index) in luckyDays" :key="index" class="overflow-hidden rounded-2xl bg-white shadow-sm">
          <!-- 日期头部 -->
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <div v-if="day.isWeekend"
                       class="h-4 w-4 flex items-center justify-center rounded-full bg-purple-500 text-white"
                  >
                    <span class="text-xs">休</span>
                  </div>
                </div>
                <div class="mt-0.5 text-sm text-gray-500">
                  {{ day.lunarDay.getLunarMonth() }} {{ day.lunarDay.getName() }}
                </div>
              </div>
            </div>
            <!-- 干支 -->
          ...
          <div class="from-green-50/50 to-emerald-50/50 bg-gradient-to-r px-4 py-3">
            ...
            <div class="flex flex-wrap gap-2">
              <template v-for="event in day.matchedEvents" :key="event">
                <div v-if="getEventConfig(event)" class="flex items-center gap-1 rounded-full px-2.5 py-1"
          ...
          <div class="px-4 py-3">
            <div class="flex flex-wrap gap-1.5">
              <span v-for="(recommend, rIndex) in day.recommends.slice(0, 8)" :key="rIndex"
                    class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500"
              >
                {{ recommend }}
              </span>
              <span v-if="day.recommends.length > 8" class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-400">
...
.bg-rose-100 {
  --un-bg-opacity: 1;
  background-color: rgb(255 228 230 / var(--un-bg-opacity));
}
...
.ring-rose-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(253 164 175 / var(--un-ring-opacity));
}

.bg-amber-100 {
  --un-bg-opacity: 1;
  background-color: rgb(254 243 199 / var(--un-bg-opacity));
}

.text-amber-600 {
  --un-text-opacity: 1;
  color: rgb(217 119 6 / var(--un-text-opacity));
}

.ring-amber-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(252 211 77 / var(--un-ring-opacity));
}

.bg-emerald-100 {
  --un-bg-opacity: 1;
  background-color: rgb(209 250 229 / var(--un-bg-opacity));
}

.text-emerald-600 {
  --un-text-opacity: 1;
  color: rgb(5 150 105 / var(--un-text-opacity));
}

.ring-emerald-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(110 231 183 / var(--un-ring-opacity));
}

.bg-sky-100 {
  --un-bg-opacity: 1;
  background-color: rgb(224 242 254 / var(--un-bg-opacity));
}

.text-sky-600 {
  --un-text-opacity: 1;
  color: rgb(2 132 199 / var(--un-text-opacity));
}

.ring-sky-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(125 211 252 / var(--un-ring-opacity));
}
...
.ring-purple-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(216 180 254 / var(--un-ring-opacity));
}

.bg-orange-100 {
  --un-bg-opacity: 1;
  background-color: rgb(255 237 213 / var(--un-bg-opacity));
}

.text-orange-600 {
  --un-text-opacity: 1;
  color: rgb(234 88 12 / var(--un-text-opacity));
}

.ring-orange-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(253 186 116 / var(--un-ring-opacity));
}

.bg-teal-100 {
  --un-bg-opacity: 1;
  background-color: rgb(204 251 241 / var(--un-bg-opacity));
}

.text-teal-600 {
  --un-text-opacity: 1;
  color: rgb(13 148 136 / var(--un-text-opacity));
}

.ring-teal-300 {
  --un-ring-opacity: 1;
  --un-ring-color: rgb(94 234 212 / var(--un-ring-opacity));
}

.bg-pink-100 {
  --un-bg-opacity: 1;
  background-color: rgb(252 231 243 / var(--un-bg-opacity));
}

.text-pink-600 {
  --un-text-opacity: 1;
  color: rgb(219 39 119 / var(--un-text-opacity));
}
